...
export default function AppointmentsPage() {
  ...
  const [filter, setFilter] = useState<'all' | 'attended' | 'nonAttended'>('all');
      ...
      <div className="">
        ...
        <div className="mb-4 flex justify-center gap-1">
          <button
            ...
            className={`px-4 py-2 border rounded-lg ${filter === 'all' ? 'bg-blue-600 text-white' : 'bg-white text-blue-600 border-blue-600'} transition-all duration-300`}
          >
            All
          </button>
          <button
            onClick={() => setFilter('attended')}
            className={`px-4 py-2 border rounded-lg ${filter === 'attended' ? 'bg-green-600 text-white' : 'bg-white text-green-600 border-green-600'} transition-all duration-300`}
          >
            Attended
          </button>
          <button
            onClick={() => setFilter('nonAttended')}
            className={`px-4 py-2 border rounded-lg ${filter === 'nonAttended' ? 'bg-red-600 text-white' : 'bg-white text-red-600 border-red-600'} transition-all duration-300`}
          >
            Not Attended
          </button>
        </div>
          ...
          <table className="min-w-full bg-white border border-gray-200 divide-y divide-gray-200 table-fixed">
            <thead className="bg-[white] text-center">
              <tr>
                ...
                <th className="py-3 px-2 text-left text-xs md:text-[16px] leading-5 font-bold text-black w-28">Status</th>
              </tr>
            </thead>
            <tbody>
